-- Security fixes: Enhanced validation, device sessions RLS, role security (without vault)
...
-- Enhanced PIX key storage with hash-based security (without vault)
CREATE OR REPLACE FUNCTION public.store_encrypted_pix_key_enhanced(
...
BEGIN
  ...
  -- Create a secure hash of the PIX key with salt
  pix_hash := encode(digest(p_pix_key || gen_random_uuid()::text, 'sha256'), 'hex');
  
  -- Store in encrypted_pix_data table with enhanced hash
  INSERT INTO encrypted_pix_data (withdraw_request_id, encrypted_pix_key)
  VALUES (p_withdraw_request_id, pix_hash)
  RETURNING id INTO encrypted_id;
  
  -- Log admin action
  PERFORM log_admin_action(
    'PIX_KEY_STORED_ENHANCED',
    'encrypted_pix_data',
    encrypted_id,
    jsonb_build_object(
      'withdraw_request_id', p_withdraw_request_id,
      'hash_length', length(pix_hash)
    )
  );
