-- Add tutorial_skipped column to profiles table
ALTER TABLE public.profiles ADD COLUMN tutorial_skipped boolean DEFAULT false;

-- Update purchase_sprite_with_lottery function to fix the lottery bug
...
BEGIN
  ...
  -- Check if buyer already owns this sprite
  SELECT EXISTS(
  ...
  IF existing_owners IS NOT NULL AND array_length(existing_owners, 1) > 0 THEN
    IF buyer_already_owns THEN
      -- Buyer already owns sprite, exclude them from lottery
      -- Only other owners participate in lottery
      lottery_participants := existing_owners;
    ELSE
      -- Buyer doesn't own sprite, add them to lottery participants
      lottery_participants := existing_owners || p_buyer_user_id;
    END IF;
    
    -- Random selection from lottery participants
