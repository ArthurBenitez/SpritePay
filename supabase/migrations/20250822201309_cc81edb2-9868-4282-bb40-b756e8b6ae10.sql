-- Update process_referral_reward function to use correct point-based milestones
...
BEGIN
  ...
  IF p_milestone_type NOT IN ('first_withdrawal', 'withdrawal_25', 'withdrawal_100', 'withdrawal_500', 'withdrawal_1000') THEN
  ...
  CASE p_milestone_type
    ...
    WHEN 'withdrawal_100' THEN milestone_valid := withdrawal_points >= 100;
    WHEN 'withdrawal_500' THEN milestone_valid := withdrawal_points >= 500;
    WHEN 'withdrawal_1000' THEN milestone_valid := withdrawal_points >= 1000;
  END CASE;
  ...
  VALUES (
    ...
    CASE p_milestone_type
      ...
      WHEN 'withdrawal_100' THEN 'atingiu 100 pontos em saques!'
      WHEN 'withdrawal_500' THEN 'atingiu 500 pontos em saques!'
      WHEN 'withdrawal_1000' THEN 'atingiu 1000 pontos em saques!'
...
-- Update approve_withdrawal_secure function to use correct milestones
...
BEGIN
  ...
  BEGIN
    ...
    IF withdrawal_points >= 100 THEN
      PERFORM process_referral_reward(
        withdrawal_record.user_id,
        'withdrawal_100',
        withdrawal_record.amount
      );
      RAISE NOTICE 'Processed withdrawal_100 milestone for user %', withdrawal_record.user_id;
    END IF;
    ...
    IF withdrawal_points >= 1000 THEN
      PERFORM process_referral_reward(
        withdrawal_record.user_id,
        'withdrawal_1000',
        withdrawal_record.amount
      );
      RAISE NOTICE 'Processed withdrawal_1000 milestone for user %', withdrawal_record.user_id;
    END IF;
  EXCEPTION WHEN OTHERS THEN
