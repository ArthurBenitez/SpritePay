...
BEGIN
  ...
  AND view_timestamp > current_date;
  
  IF daily_views_count >= ad_config.max_daily_views THEN
    RETURN jsonb_build_object(
      'success', false,
  ...
  AND view_timestamp > now() - INTERVAL '1 minute' * ad_config.cooldown_minutes;
  ...
  -- Check if user should receive credits (every X views)
  SELECT COUNT(*) INTO daily_views_count
  FROM ad_impressions
  WHERE user_id = current_user_id
  AND view_timestamp > current_date;
  ...
  IF daily_views_count % ad_config.ads_per_credit = 0 THEN
    credits_to_award := 1;
  ...
  views_needed := ad_config.ads_per_credit - (daily_views_count % ad_config.ads_per_credit);
  IF views_needed = ad_config.ads_per_credit THEN
    views_needed := 0;
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'ads_per_credit', ad_config.ads_per_credit,
    'max_daily_views', ad_config.max_daily_views
  );
...
-- Function to get ad statistics for users
CREATE OR REPLACE FUNCTION public.get_user_ad_stats(p_user_id uuid DEFAULT NULL)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = 'public'
...
BEGIN
  ...
  IF target_user_id != auth.uid() AND NOT is_current_user_admin() THEN
  ...
  -- Calculate views needed for next credit
  views_needed := ad_config.ads_per_credit - (daily_views % ad_config.ads_per_credit);
  IF views_needed = ad_config.ads_per_credit THEN
    views_needed := 0;
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'ads_per_credit', ad_config.ads_per_credit,
    'max_daily_views', ad_config.max_daily_views,
    'progress_percentage', ROUND((daily_views % ad_config.ads_per_credit) * 100.0 / ad_config.ads_per_credit, 1)
  );
END;
$$;